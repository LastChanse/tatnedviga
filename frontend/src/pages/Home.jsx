...
export default function HomePage() {
  ...
  const statusMeta = (status) => {
    switch (status) {
      ...
      case "booked":
        return { label: "Забронирован", cls: "bg-amber-50 text-amber-700" };
      case "sold":
        return { label: "Продан", cls: "bg-rose-50 text-rose-700" };
      case "rented":
        return { label: "Сдан в аренду", cls: "bg-sky-50 text-sky-700" };
      default:
          ...
          <div className="grid gap-6 lg:grid-cols-[1fr,360px] lg:items-start">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-gray-600">
                Фильтруйте объекты по району, цене, статусу и типу недвижимости.
              </p>
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                <input
                  ...
                  placeholder="Поиск по объектам, адресу, району…"
                  value={query}
          ...
          <div className="mb-4 flex flex-wrap items-baseline justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                {showOnlyAvailable ? "Показаны только активные и доступные объекты" : "Показаны все объекты, включая неактивные и завершённые"}
              </p>
            </div>
            ...
            <aside className="top-[76px] h-fit rounded-2xl border border-gray-200 bg-white p-4 lg:sticky">
              ...
              <div className="mb-4 rounded-2xl bg-gray-100 p-1.5">
                <button
                  ...
                  className={showOnlyAvailable ? activeFilterButton : inactiveFilterButton}
                >
                  {showOnlyAvailable ? "✓ Только активные и доступные" : "Показать все объекты"}
                </button>
              </div>
              ...
              <button className="mt-4 w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-sm font-extrabold text-gray-900 hover:bg-gray-100" type="button" onClick={resetFilters}>
                Сбросить фильтры
              </button>
            </aside>
            ...
            <section className="min-w-0">
                      ...
                      <Link to={`/property/${item.id}`}>
                        ...
                        <div className="p-4">
                          ...
                          <div className="mt-1 text-sm text-gray-500">
                            {typeLabels[item.property_type]} • {item.deal === 'rent' ? 'Аренда' : 'Продажа'}{item.district ? ` • ${item.district}` : ''}
                          </div>

                          {isOwner ? (
                            <Link to="/owner/properties" className="mt-4 block w-full rounded-xl border border-gray-900 bg-white px-3 py-2 text-center text-sm font-extrabold text-gray-900 hover:bg-gray-50" onClick={(e) => e.stopPropagation()}>
                              Управлять объявлениями
                            </Link>
                          ) : (
                            <button
                              className={canClientAct ? "mt-4 w-full rounded-xl border border-gray-900 bg-white px-3 py-2 text-sm font-extrabold text-gray-900 hover:bg-gray-50" : "mt-4 w-full rounded-xl border border-gray-200 bg-gray-100 px-3 py-2 text-sm font-extrabold text-gray-500"}
                              type="button"
                              disabled={!canClientAct}
                              onClick={(e) => {
                                e.preventDefault();
                                if (!isAuthed) return setAuthModalOpen(true);
                                if (canClientAct) window.location.href = `/property/${item.id}`;
                              }}
                            >
                              {canClientAct ? "Записаться / подробнее" : "Недоступно"}
                            </button>
                          )}
                        </div>
                      </Link>
              ...
              {filtered.length === 0 && (
                <div className="mt-4 rounded-2xl border border-dashed border-gray-300 p-5 text-sm text-gray-600">
                  ...
                  <div className="mt-1">Попробуйте изменить фильтры или выключить фильтр активных доступных объектов.</div>
                </div>
              )}
            </section>
          </div>
      ...
      {authModalOpen && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-lg">
            ...
            <p className="mt-1 text-sm text-gray-600">Чтобы добавить объект в избранное или записаться на просмотр, войдите или зарегистрируйтесь.</p>
            <div className="mt-4 flex gap-2">
              <Link to="/login" className="flex-1 rounded-xl border border-gray-200 px-3 py-2 text-center text-sm font-extrabold text-gray-900 hover:bg-gray-50">Войти</Link>
              <Link to="/register" className="flex-1 rounded-xl bg-gray-900 px-3 py-2 text-center text-sm font-extrabold text-white hover:bg-gray-800">Регистрация</Link>
            </div>
